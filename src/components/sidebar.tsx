...
import { LayoutDashboard, ListTodo, Bot, FolderKanban, LogOut } from "lucide-react";
              ...
              <li key={item.href}>
                <Link
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${
                    active
                      ? "bg-blue-50/80 text-blue-700 nav-active"
                      : "text-gray-500 hover:bg-gray-50 hover:text-gray-900"
                  }`}
                >
                  <item.icon className={`w-4 h-4 transition-colors ${active ? "text-blue-600" : "text-gray-400"}`} />
                  {item.label}
                </Link>
              </li>
            );
